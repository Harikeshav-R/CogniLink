...
async def object_permanence_workflow_runner(broadcaster: FrameBroadcaster) -> None:
    ...
    logger.trace(f"Initial runner state: last_processed_version={last_processed_version}, previous_frame_objects=[], previous_room=None")
    ...
    while broadcaster.is_running:
        ...
        try:
            ...
            async with get_session() as db_session:
                ...
                logger.trace(f"Initial workflow state prepared for version {version}: {initial_state.model_dump(exclude={'db_session', 'current_frame_b64'})}")
                ...
                logger.trace(f"Final workflow state: {final_state.model_dump(exclude={'db_session', 'current_frame_b64'})}")
            ...
            logger.error(f"An unhandled error occurred while processing frame version {version}: {e}", exc_info=True, backtrace=True)
