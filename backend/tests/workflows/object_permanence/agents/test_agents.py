...
def mock_state():
    ...
    state.db_session = MagicMock(spec=AsyncSession)
...
async def test_check_frame_similarity_returns_true(mock_compare, mock_state):
    mock_compare.return_value = True
    result = await check_frame_similarity(mock_state)
...
@pytest.mark.asyncio
@patch("app.workflows.object_permanence.agents.check_frame_similarity.compare_images")
async def test_check_frame_similarity_returns_false(mock_compare, mock_state):
...
@pytest.mark.asyncio
async def test_check_frame_similarity_no_frames(mock_state):
    mock_state.previous_frame = None
    assert await check_frame_similarity(mock_state) == {}
...
async def test_analyze_static_frame_happy_path(mock_create_agent, mock_init_model, mock_state):
    mock_agent = MagicMock()
    mock_agent.ainvoke = AsyncMock(return_value={"structured_response": "test_analysis"})
    mock_create_agent.return_value = mock_agent

    result = await analyze_static_frame(mock_state)
...
@pytest.mark.asyncio
async def test_analyze_static_frame_no_frame(mock_state):
    mock_state.current_frame = None
    assert await analyze_static_frame(mock_state) == {}
...
async def test_analyze_diff_frames_happy_path(mock_create_agent, mock_init_model, mock_state):
    mock_agent = MagicMock()
    mock_agent.ainvoke = AsyncMock(return_value={"structured_response": "diff_analysis"})
    mock_create_agent.return_value = mock_agent


    result = await analyze_diff_frames(mock_state)
...
@pytest.mark.asyncio
async def test_analyze_diff_frames_no_frames(mock_state):
    mock_state.previous_frame = None
    assert await analyze_diff_frames(mock_state) == {}
...
async def test_filter_results_happy_path(mock_create_agent, mock_init_model, mock_state):
    mock_agent = MagicMock()
    mock_agent.ainvoke = AsyncMock(return_value={"structured_response": FilteredResults(entries=[FilteredEntry(content="Filtered Test", object_name="filtered_test", log_type="state")])})
    mock_create_agent.return_value = mock_agent

    result = await filter_results(mock_state)
...
@pytest.mark.asyncio
async def test_filter_results_no_analysis(mock_state):
    mock_state.static_analysis = None
    assert await filter_results(mock_state) == {}
...
async def test_save_analysis_happy_path(mock_embeddings, mock_create_log, mock_state):
    mock_embed_instance = MagicMock()
    mock_embed_instance.aembed_documents = AsyncMock(return_value=[[0.1]])
    mock_embeddings.return_value = mock_embed_instance

    result = await save_analysis(mock_state)

    mock_embeddings.assert_called_once()
    mock_embed_instance.aembed_documents.assert_called_with(["Test"])
    mock_create_log.assert_called_once()
    assert result == {"save_status": True}


@pytest.mark.asyncio
async def test_save_analysis_no_results(mock_state):
    mock_state.filtered_results = None
    assert await save_analysis(mock_state) == {}

@pytest.mark.asyncio
async def test_save_analysis_empty_results(mock_state):
